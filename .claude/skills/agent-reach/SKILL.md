...
The CLI `agent-reach` must be on PATH. Before the first `agent-reach` command,
check and install if missing:

```bash
command -v agent-reach >/dev/null 2>&1 || \
  pipx install https://github.com/Panniantong/agent-reach/archive/main.zip || \
  pip install --user https://github.com/Panniantong/agent-reach/archive/main.zip
```

If `pipx`/`pip` is unavailable or blocked by network policy, tell the user and
stop — do NOT use `sudo` or modify system files. Upstream tools (OpenCLI,
yt-dlp, gh, bili-cli, etc.) auto-install on demand per the references; never
install packages not listed in the guides. Full guide:
